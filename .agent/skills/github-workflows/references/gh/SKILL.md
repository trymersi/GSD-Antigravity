...
```bash
gh <command> -R trymersi/GSD-Antigravity
```
...
```bash
# List open PRs
gh pr list -R trymersi/GSD-Antigravity

# View PR details
gh pr view <number> -R trymersi/GSD-Antigravity

# Check PR CI status
gh pr checks <number> -R trymersi/GSD-Antigravity

# Create PR
gh pr create -R trymersi/GSD-Antigravity --title "title" --body "body"

# View PR comments
gh api repos/trymersi/GSD-Antigravity/pulls/<number>/comments
```
...
```bash
# List issues
gh issue list -R trymersi/GSD-Antigravity

# List by label
gh issue list -R trymersi/GSD-Antigravity --label "priority:p1" --state open
...
gh issue create -R trymersi/GSD-Antigravity \
  --title "feat: add feature X" \
  --label "priority:p1" \
  --milestone "v1.0"

# View issue
gh issue view <number> -R trymersi/GSD-Antigravity

# Close issue with comment
gh issue close <number> -R trymersi/GSD-Antigravity --comment "Implemented in PR #N"

# Edit labels on issue
gh issue edit <number> -R trymersi/GSD-Antigravity \
...
ISSUE_URL=$(gh issue create -R trymersi/GSD-Antigravity \
  --title "..." --body "...")

# Step 2: Set the issue type via GraphQL
ISSUE_NUM=$(echo "$ISSUE_URL" | grep -oE '[0-9]+$')
ISSUE_ID=$(gh api graphql -f query='{ repository(owner:"trymersi",name:"GSD-Antigravity") { issue(number:'"$ISSUE_NUM"') { id } } }' --jq '.data.repository.issue.id')
TYPE_ID=$(gh api graphql -f query='{ repository(owner:"trymersi",name:"GSD-Antigravity") { issueTypes(first:20) { nodes { id name } } } }' --jq '.data.repository.issueTypes.nodes[] | select(.name=="Bug") | .id')
...
```bash
# List all labels
gh label list -R trymersi/GSD-Antigravity

# Create label
gh label create "priority:p1" --color "E99695" \
  --description "High priority" -R trymersi/GSD-Antigravity
```
...
```bash
# List projects
gh project list --owner trymersi

# Create project
gh project create --owner trymersi --title "gsd-2 Backlog"

# Add issue to project
gh project item-add 1 --owner trymersi \
  --url https://github.com/trymersi/GSD-Antigravity/issues/42
```
...
```bash
# List milestones
gh api repos/trymersi/GSD-Antigravity/milestones
...
gh api repos/trymersi/GSD-Antigravity/issues/42 \
...
```bash
# List recent runs
gh run list -R trymersi/GSD-Antigravity --limit 5

# View specific run
gh run view <run-id> -R trymersi/GSD-Antigravity

# View failed job logs
gh run view <run-id> -R trymersi/GSD-Antigravity --log-failed
```
...
```bash
# List releases
gh release list -R trymersi/GSD-Antigravity

# View latest release
gh release view --repo trymersi/GSD-Antigravity
```
...
```bash
# GET request
gh api repos/trymersi/GSD-Antigravity

# POST with fields
gh api repos/trymersi/GSD-Antigravity/issues -f title="Bug" -f body="Details"

# GraphQL
gh api graphql -f query='{ viewer { login } }'

# Paginated results
gh api repos/trymersi/GSD-Antigravity/contributors --paginate
```
...
```bash
# Clone
gh repo clone trymersi/GSD-Antigravity

# View repo info
gh repo view -R trymersi/GSD-Antigravity
```
...
```bash
# JSON output
gh pr list -R trymersi/GSD-Antigravity --json number,title,state

# JQ filtering
gh pr list -R trymersi/GSD-Antigravity --json number,title --jq '.[].title'

# Template formatting
gh pr list -R trymersi/GSD-Antigravity --json number,title \
